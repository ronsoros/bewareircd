(*
 *  beware ircd, Internet Relay Chat server, b_wallops.pas
...
procedure m_wallops(cptr,sptr:tuser;parc:integer;parv:pparams);
...
begin
  ...
  sendto_serversbutone(sptr,sprefix(sptr,TOK_WALLOPS)+':'+parv[parc-1]);

  if opt.secretwallops then begin
    ...
    for a := 0 to highconnection do if connectionlist[a].open then begin
      ...
      if flag_isset(us.modeflag,usermode_wallops) then if isclient(us) then if isprivileged(us) then sendto_one(us,s);
    end;
  end else begin
    s := cprefix(sptr,MSG_WALLOPS)+':'+parv[parc-1];
    for a := 0 to highconnection do if connectionlist[a].open then begin
